...
class MapInitState(nn.Module):
    def __init__(self, input_size, hidden_size, dropout_ratio):
        super().__init__()
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.projectionh = nn.Linear(input_size, hidden_size)
        self.projectionc = nn.Linear(input_size, hidden_size)
        self.drop = nn.Dropout(dropout_ratio)

    def forward(self, h, c):
        h = self.drop(self.projectionh(h))
        c = self.drop(self.projectionc(c))
