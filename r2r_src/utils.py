...
def load_datasets(splits):
    ...
    for split in splits:
        ...
        if "/" not in split:
            if args.dataset == 'RxR':
                if split == 'test':
                    with open('../RXR/rxr-data/rxr_test_standard_public_guide.jsonl') as f:
                        for line in f:
                            new_data.append(json.loads(line))
                else:
                    with open('../RXR/rxr-data/rxr_%s_guide.jsonl' % split) as f:
                        for line in f:
                            new_data.append(json.loads(line))
...
class Tokenizer(object):
    ...
    def __init__(self, vocab=None, encoding_length=20):
        ...
        # if args.train == 'speaker_transformer':
        self.add_word('<VID>')
        self.add_word('<SEP>')
        self.add_word('<CLS>')
        assert self.vocab_size() == old+4
    ...
    def encode_sentence(self, sentence, max_length=None):
        ...
        if len(encoding) < max_length:
            ...
            text_mask += [0] * (max_length - current_length)
        elif len(encoding) > max_length:
            encoding[max_length - 1] = self.word_to_index['<EOS>']                  # Cut the length with EOS
            text_mask[max_length - 1] = 1

        return np.array(encoding[:max_length]), np.array(text_mask[:max_length])
    ...
    def encode(self, input, max_length=None):
        output = []

        for token in input:
            output.append(self.word_to_index[token])

        # assert len(output) == args.maxAction
...
def load_style_embedding(style_store):
    import csv
    import base64
    from tqdm import tqdm

    print("Start loading the style embedding")
    start = time.time()
    ...
    tsv_fieldnames = ['scanId', 'viewpointId', 'id', 'features']
    features = {}
    with open(style_store, "r") as tsv_in_file:     # Open the tsv file.
        reader = csv.DictReader(tsv_in_file, delimiter='\t', fieldnames=tsv_fieldnames)
        for item in reader:
            long_id = item['scanId'] + "_" + item['viewpointId'] + "_" + item['id']
            features[long_id] = np.frombuffer(base64.decodestring(item['features'].encode('ascii')),
                                                   dtype=np.float32)   # Feature of long_id is (36, 2048)
            # print(features[long_id].shape)
            # exit()

    print("Finish Loading the image feature from %s in %0.4f seconds" % (style_store, time.time() - start))
    return features
