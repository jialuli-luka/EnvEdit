...
class Speaker():
    ...
    def __init__(self, env, listener, tok):
        ...
        # Model
        print("VOCAB_SIZE", self.tok.vocab_size()-3)
        ...
        self.decoder = model.SpeakerDecoder(self.tok.vocab_size()-3, args.wemb, self.tok.word_to_index['<PAD>'],
        ...
        if args.style_embedding:
            self.map_style = model.MapInitState(256, args.rnn_dim, args.dropout).cuda()
            self.map_style_optimizer = args.optimizer(self.map_style.parameters(), lr=args.lr)
    ...
    def train(self, iters):
        for i in range(iters):
            ...
            if args.style_embedding:
                self.map_style_optimizer.zero_grad()
            ...
            if args.style_embedding:
                torch.nn.utils.clip_grad_norm(self.map_style.parameters(), 40.)
                self.map_style_optimizer.step()
    ...
    def get_style_feats(self, obs):
        style_feat = np.zeros((len(obs), 256), dtype=np.float32)
        for i, ob in enumerate(obs):
            style_feat[i,:] = ob['style']

        return torch.from_numpy(style_feat).cuda()

    def from_shortest_path(self, viewpoints=None, get_first_feat=False, get_style=False):
        """
        ...
        while not ended.all():
            ...
            if get_style:
                style_feats.append(self.get_style_feats(obs))
        ...
        if get_style:
            style_feats = torch.stack(style_feats, 1).contiguous()
            return (img_feats, can_feats), length, style_feats
    ...
    def teacher_forcing(self, train=True, features=None, insts=None, for_listener=False):
        ...
        if features is not None:
            ...
            batch_size = len(obs)
            if args.style_embedding:
                (img_feats, can_feats), lengths, style_embedding = self.from_shortest_path(get_style=True)  # Image Feature (from the shortest path)
            else:
                (img_feats, can_feats), lengths = self.from_shortest_path()      # Image Feature (from the shortest path)
            ctx = self.encoder(can_feats, img_feats, lengths)

        if args.style == 'first':
            style_embedding = self.get_style_feats(obs).unsqueeze(0)
            h_t, c_t = self.map_style(style_embedding, style_embedding)
        elif args.style == 'all':
            style_embedding = torch.mean(style_embedding, dim=1).unsqueeze(0)
            h_t, c_t = self.map_style(style_embedding, style_embedding)
        else:
    ...
    def infer_batch(self, sampling=False, train=False, featdropmask=None):
        ...
        # Get feature
        if args.style_embedding:
            (img_feats, can_feats), lengths, style_embedding = self.from_shortest_path(viewpoints=viewpoints_list, get_style=True)  # Image Feature (from the shortest path)
        else:
    ...
    def save(self, epoch, path):
        ...
        if args.style_embedding:
            all_tuple.append(("map_style", self.map_style, self.map_style_optimizer))
