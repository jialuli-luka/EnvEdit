...
class Seq2SeqAgent(BaseAgent):
    ...
    def __init__(self, env, results_path, tok, episode_len=20):
        ...
        self.encoder = model.EncoderLSTM(tok.vocab_size()-3, args.wemb, enc_hidden_size, padding_idx,
    ...
    def ensemble_test(self, use_dropout=False, feedback='argmax', iters=None, agent2=None, agent3=None):
        ...
        if iters is not None:
            # For each time, it will run the first 'iters' iterations. (It was shuffled before)
            for i in range(iters):
                for traj in self.ensemble_rollout(agent2, agent3):
                    self.loss = 0
                    self.results[traj['instr_id']] = traj['path']
        else:  # Do a full round
            while True:
    ...
    def ensemble_rollout(self, agent2=None, agent3=None):
        obs = np.array(self.env.reset())
        ...
        ctx, h_t, c_t = self.encoder(seq, seq_lengths)
        ctx1, h_t1, c_t1 = agent2.encoder(seq, seq_lengths)
        ctx2, h_t2, c_t2 = agent3.encoder(seq, seq_lengths)
        ctx_mask = seq_mask
        ...
        for t in range(self.episode_len):
            ...
            h_t1, c_t1, logit1, h11 = agent2.decoder(input_a_t, f_t, candidate_feat, h_t1, h11, c_t1, ctx1, ctx_mask,
                                                     already_dropfeat=False)

            input_a_t, f_t, candidate_feat, candidate_leng = self.get_input_feat(perm_obs)
            h_t2, c_t2, logit2, h12 = agent3.decoder(input_a_t, f_t, candidate_feat, h_t2, h12, c_t2, ctx2, ctx_mask,
                                                     already_dropfeat=False)
            ...
            logit.masked_fill_(candidate_mask, -float('inf'))
            logit1.masked_fill_(candidate_mask, -float('inf'))
            logit2.masked_fill_(candidate_mask, -float('inf'))

            logit_ = (logit1 + logit2 + logit) / 3

            _, a_t = logit_.max(1)        # student forcing - argmax
            a_t = a_t.detach()
